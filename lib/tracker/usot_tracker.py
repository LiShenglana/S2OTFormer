...
class USOTTracker(object):
    ...
    def init(self, im_color, im_ir, target_pos, target_sz, model):
        ...
        avg_chans_color = np.mean(im_color, axis=(0, 1))
        avg_chans_ir = np.mean(im_ir, axis=(0, 1))
        z_crop_color, crop_info = get_subwindow_tracking(im_color, target_pos, p.exemplar_size,
                                                   s_z, avg_chans_color, target_sz, need_bbox=True, vis=False)
        z_crop_ir, _ = get_subwindow_tracking(im_ir, target_pos, p.exemplar_size,
                                                   s_z, avg_chans_ir, target_sz, need_bbox=True, vis=False)
        ...
        z_color = z_crop_color.unsqueeze(0)
        z_ir = z_crop_ir.unsqueeze(0)
        net.template(z_color.cuda(), z_ir.cuda(), template_bbox=template_bbox)
        ...
        state['p'] = p
        state['net'] = net
        state['avg_chans_color'] = avg_chans_color
        state['avg_chans_ir'] = avg_chans_ir
        state['window'] = window
        ...
        x_crop_color, crop_info = get_subwindow_tracking(im_color, target_pos, p.instance_size,
                                                   python2round(s_x), avg_chans_color, target_sz,
                                                   out_mode='raw', need_bbox=True, vis=False)
        x_crop_ir, _ = get_subwindow_tracking(im_ir, target_pos, p.instance_size,
                                                         python2round(s_x), avg_chans_ir, target_sz,
                                                         out_mode='raw', need_bbox=True, vis=False)
        search_bbox = crop_info['template_bbox']
        x_crop_unsq_color = im_to_torch(x_crop_color.copy())
        x_crop_unsq_color = x_crop_unsq_color.unsqueeze(0)
        x_crop_unsq_ir = im_to_torch(x_crop_ir.copy())
        x_crop_unsq_ir = x_crop_unsq_ir.unsqueeze(0)
        search_bbox_pool = self.pool_label_search(p, search_bbox)
        search_bbox_pool = torch.tensor([search_bbox_pool]).float().cuda()
        if net.modality == 'RGB-T':
            # memory_feature_color = net.extract_memory_feature_ResNet(ori_x=x_crop_unsq_color.cuda(),
            #                                                          search_bbox=search_bbox_pool, img='RGB')
            # memory_feature_ir = net.extract_memory_feature_ResNet(ori_x=x_crop_unsq_ir.cuda(),
            #                                                       search_bbox=search_bbox_pool, img='T')
            # if net.fuse_method == 'Add':
            #     memory_feature = torch.add(memory_feature_color, memory_feature_ir)
            # elif net.fuse_method == 'Cross_Attention':
            #     _, _, _, Wx = memory_feature_color.shape
            #     memory_feature = net.featurefusion_network(net.input_proj(memory_feature_ir),
            #                                                net.input_proj(memory_feature_color), Wx)
            memory_feature = net.extract_memory_feature_ResNet_fuse(ori_x_color=x_crop_unsq_color.cuda(),
                                                                        ori_x_ir=x_crop_unsq_ir.cuda(),
                                                                        search_bbox=search_bbox_pool)
        elif net.modality == 'RGB':
            memory_feature = net.extract_memory_feature_ResNet(ori_x=x_crop_unsq_color.cuda(),
                                                                     search_bbox=search_bbox_pool, img='RGB')
        elif net.modality == 'T':
            memory_feature = net.extract_memory_feature_ResNet(ori_x=x_crop_unsq_ir.cuda(),
                                                                  search_bbox=search_bbox_pool, img='T')
            # memory_feature = memory_feature_color
        # elif p.Feature_Backbone == 'Vit':
        #     memory_feature = net.extract_memory_feature(ori_x_ir=x_crop_unsq_ir.cuda(), ori_x_color=x_crop_unsq_color.cuda(), temp=False, search_bbox=search_bbox_pool)
        # memory_feature = net.neck(memory_feature, crop=False)
        # memory_feature = memory_feature_color
        ...
        search_bbox_aug = [self.clip_number(search_bbox_aug[0].x1, _max=x_crop_aug_color.shape[0]),
                           ...
                           self.clip_number(search_bbox_aug[0].y2, _max=x_crop_aug_color.shape[1])]
        x_crop_aug_color = im_to_torch(x_crop_aug_color.copy())
        x_crop_aug_unsq_color = x_crop_aug_color.unsqueeze(0)
        x_crop_aug_ir = im_to_torch(x_crop_aug_ir.copy())
        x_crop_aug_unsq_ir = x_crop_aug_ir.unsqueeze(0)
        ...
        if net.modality == 'RGB-T':
            # memory_feature_aug_color = net.extract_memory_feature_ResNet_fuse(ori_x=x_crop_aug_unsq_color.cuda(),
            #                                                              search_bbox=search_bbox_aug_pool, img='RGB')
            # memory_feature_aug_ir = net.extract_memory_feature_ResNet_fuse(ori_x=x_crop_aug_unsq_ir.cuda(),
            #                                                           search_bbox=search_bbox_aug_pool, img='T')
            # if net.fuse_method == 'Add':
            #     memory_feature_aug = torch.add(memory_feature_aug_color, memory_feature_aug_ir)
            # elif net.fuse_method == 'Cross_Attention':
            #     _, _, _, Wx = memory_feature_aug_color.shape
            #     memory_feature_aug = net.featurefusion_network(net.input_proj(memory_feature_aug_ir),
            #                                                    net.input_proj(memory_feature_aug_color), Wx)
            # memory_feature_aug = net.neck(memory_feature_aug, crop=False)
            memory_feature_aug = net.extract_memory_feature_ResNet_fuse(ori_x_color=x_crop_aug_unsq_color.cuda(), ori_x_ir=x_crop_aug_unsq_ir.cuda(),
                                                                  search_bbox=search_bbox_aug_pool)
        elif net.modality == 'RGB':
            memory_feature_aug = net.extract_memory_feature_ResNet(ori_x=x_crop_aug_unsq_color.cuda(),
                                                                         search_bbox=search_bbox_aug_pool, img='RGB')
        elif net.modality == 'T':
            memory_feature_aug = net.extract_memory_feature_ResNet(ori_x=x_crop_aug_unsq_ir.cuda(),
                                                                      search_bbox=search_bbox_aug_pool, img='T')
        # elif p.Feature_Backbone == 'Vit':
        #     memory_feature_aug = net.extract_memory_feature(ori_x_ir=x_crop_aug_unsq_ir.cuda(),
        #                                                 ori_x_color=x_crop_aug_unsq_color.cuda(), temp=False, search_bbox=search_bbox_aug_pool)
        # memory_feature_aug = net.neck(memory_feature_aug, crop=False)
    ...
    def update(self, net, x_crops_color, x_crops_ir, target_pos, target_sz, window,
    ...
    def track(self, state, im_color, im_ir):
        p = state['p']
        net = state['net']
        avg_chans_color = state['avg_chans_color']
        avg_chans_ir = state['avg_chans_ir']
        window = state['window']
        ...
        x_crop_color, _ = get_subwindow_tracking(im_color, target_pos, p.instance_size, python2round(s_x), avg_chans_color)
        x_crop_color = x_crop_color.unsqueeze(0)
        x_crop_ir, _ = get_subwindow_tracking(im_ir, target_pos, p.instance_size, python2round(s_x), avg_chans_ir)
        x_crop_ir = x_crop_ir.unsqueeze(0)
...
class USOTConfig(object):
    ...
    lr = 0.730
    windowing = 'cosine'
    exemplar_size = 128 #127
    instance_size = 256 #255
    total_stride = 8
