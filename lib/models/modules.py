...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, dilation=1):
        ...
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=stride,
                               padding=padding, bias=False, dilation=dilation)   # padding
...
class ResNet_plus2(nn.Module):
    ...
    def __init__(self, block, layers, used_layers):
        ...
        self.conv1 = nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=0,  # 0->3
                               bias=False)
        self.bn1 = nn.BatchNorm2d(64)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)  # 1
        # stride = [1 + (dilation_factor < l) for l in (8, 4, 2)]
        # self.layer1 = self._make_layer(block, 64, layers[0], dilation=max(dilation_factor // 8, 1))
        # self.layer2 = self._make_layer(block, 64 * 2, layers[1], stride=stride[0],
