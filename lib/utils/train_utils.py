...
def load_pretrain(model, pretrained_path, print_unuse=True, gpus=None):
    ...
    if "imagenet" in pretrained_path:
        ...
        ori_keys = list(pretrained_dict.keys())
        for ori_key in ori_keys:
            if "features.features" in ori_key:
                revised_key = ori_key.replace("features.features", "features")
                processed_dict[revised_key] = pretrained_dict[ori_key]
        try:
            check_keys(model.backbone_net_RGB, processed_dict, print_unuse=print_unuse)
            model.backbone_net_RGB.load_state_dict(processed_dict, strict=False)
        except:
            pass

        try:
            check_keys(model.backbone_net_stage, processed_dict, print_unuse=print_unuse)
            model.backbone_net_stage.load_state_dict(processed_dict, strict=False)
        except:
            pass

        try:
            check_keys(model.backbone_net_T, processed_dict, print_unuse=print_unuse)
            model.backbone_net_T.load_state_dict(processed_dict, strict=False)
        except:
            pass
        # check_keys(model.backbone_net_RGB, pretrained_dict, print_unuse=print_unuse)
        # model.backbone_net_RGB.load_state_dict(pretrained_dict, strict=False)
    return model

def load_pretrain_test(model, pretrained_path, print_unuse=True, gpus=None):
    print('load pretrained model from {}'.format(pretrained_path))
