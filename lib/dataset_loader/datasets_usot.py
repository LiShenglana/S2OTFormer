...
class USOTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        search_image_color = cv2.imread(pair_info[0])
        search_image_ir = cv2.imread(pair_info[1])
        search_bbox = self._toBBox(search_image_color, pair_info[2])
        ...
        if self.loader_test:
            self._draw(search_aug_color, bbox_s, "../../var/loader/" + loader_test_name + "_s.jpg")
            self._draw(template_aug_color, bbox_t, "../../var/loader/" + loader_test_name + "_t.jpg")
            #self._draw(search_aug_ir, bbox_s, "../../var/loader/" + loader_test_name + "_s.jpg")
            #self._draw(template_aug_ir, bbox_t, "../../var/loader/" + loader_test_name + "_t.jpg")
        ...
        if self.cycle_memory:
            ...
            search_bbox_nearby = [self._toBBox(search_images_nearby_color[i], pair_info[5][i])
                                  for i in range(len(search_images_nearby_color))]
            for i in range(len(search_images_nearby_color)):
                crop_nearby_color, crop_nearby_ir, bbox_nearby, _ = self._augmentation(search_images_nearby_color[i], search_images_nearby_ir[i], search_bbox_nearby[i],
                                                                 self.search_size, search=True,
                                                                 cycle_memory=self.cycle_memory)
                # Draw for loader testing
                if self.loader_test:
                    self._draw(crop_nearby_color, bbox_nearby, "../../var/loader/" +
                               loader_test_name + "_n_{:02d}.jpg".format(i))
                    self._draw(crop_nearby_ir, bbox_nearby, "../../var/loader/" +
                               loader_test_name + "_n_{:02d}.jpg".format(i))
                crop_nearby_color = np.array(crop_nearby_color)
                crop_nearby_ir = np.array(crop_nearby_ir)
                crop_nearby_color = np.transpose(crop_nearby_color, (2, 0, 1)).astype(np.float32)
                crop_nearby_ir = np.transpose(crop_nearby_ir, (2, 0, 1)).astype(np.float32)
                search_memory_color.append(crop_nearby_color)
                search_memory_ir.append(crop_nearby_ir)
            search_memory_color = np.stack(search_memory_color)
            search_memory_ir = np.stack(search_memory_ir)
            #print('memory_length:', len(search_memory_color))

        # From PIL image to numpy
        template_color = np.array(template_aug_color)
        search_color = np.array(search_aug_color)
        template_ir = np.array(template_aug_ir)
        search_ir = np.array(search_aug_ir)

        template_color, search_color = map(lambda x: np.transpose(x, (2, 0, 1)).astype(np.float32), [template_color, search_color])
        template_ir, search_ir = map(lambda x: np.transpose(x, (2, 0, 1)).astype(np.float32),
                                           [template_ir, search_ir])
        ...
        label2 = self._dynamic_label([31, 31], dag_param_s.shift)
        # out_label_ir = self._dynamic_label([self.size, self.size], dag_param_s_ir.shift)
        ...
        # visualization = True
        # if visualization:
        #     im_show = cv2.cvtColor(search_image_color, cv2.COLOR_RGB2BGR)
        #     cv2.rectangle(im_show, (int(search_bbox[0]), int(search_bbox[1])),
        #                   (int(search_bbox[2]), int(search_bbox[3])),
        #                   (0, 255, 0), 3)
        #     cv2.putText(im_show, 'bbox', (40, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2, cv2.LINE_AA)
        ...
        # bbox_t_ir = self.pool_label_template(bbox_t_ir)
        # bbox_t_ir = np.array(bbox_t_ir, np.float32)

        if len(search_memory_color) > 0:
    ...
    def _augmentation(self, image_color, image_ir, bbox, size, search=False, cycle_memory=False):
        """
...
class subData(object):
    ...
    def _get_siamese_image_anno(self, video, track_id, video_index=None):
        ...
        if video_quality_score >= video_tolerance_threshold and corner_bbox_freq < 0.25:
            ...
            if self.data_name == 'VOT2019rgbtir':
                ...
                image_path_s_color = join(self.root, video, 'color', "{}.{}.x.jpg".format(frame_id_s_format[-6:], track_id))
                image_path_s_ir = join(self.root, video, 'ir', "{}.{}.x.jpg".format(frame_id_s_format[-6:], track_id))
    ...
    def _get_cycle_memory_image_anno(self, video, track_id, video_index=None):
        ...
        if video_quality_score >= video_tolerance_threshold and corner_bbox_freq < 0.25:
            ...
            frame_id_s_format = '0' * (8 - len(frame_id_s)) + frame_id_s
            if self.data_name == 'VOT2019rgbtir':
                image_path_s_color = join(self.root, video, 'color',
                                          "{}.{}.x.jpg".format(frame_id_s_format[-6:], track_id))
                image_path_s_ir = join(self.root, video, 'ir', "{}.{}.x.jpg".format(frame_id_s_format[-6:], track_id))
            elif self.data_name == 'LasHeR':
                image_path_s_color = join(self.root, video, 'visible', "{}.{}.x.jpg".format(frame_id_s, track_id))
                image_path_s_ir = join(self.root, video, 'infrared', "{}.{}.x.jpg".format(frame_id_s, track_id))
            ...
            if self.data_name == 'VOT2019rgbtir':
                ...
                image_path_nearby_s_ir = [join(self.root, video, 'ir', "{}.{}.x.jpg".format(frame_id[-6:], track_id))
                                          for frame_id in frame_id_nearby_s_format]
            elif self.data_name == 'LasHeR':
...
if __name__ == '__main__':
    ...
    import importlib
    config_module = importlib.import_module("lib.config.config_usot")
