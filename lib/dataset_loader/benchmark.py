...
def load_dataset(dataset):
    ...
    if 'OTB2015' in dataset:
        base_path = join(realpath(dirname(__file__)), '/private/dataset/OTB100')
        json_path = join(realpath(dirname(__file__)), '/private/USOT/scripts/test', dataset + '.json')
    ...
    elif 'GTOT' in dataset:
        base_path = join('/home/cscv/Documents/lsl/dataset/', dataset)
        json_path = join('/home/cscv/Documents/lsl/dataset/', dataset + '.json')
        ...
        for f_video in father_videos:
            ...
            # ground truth
            gt_path = join(f_video_path, 'groundTruth_v.txt')
            gt = np.loadtxt(gt_path, delimiter=' ')
            # get img file
            img_path_color = join(f_video_path, 'v', '*.png')
            img_path_ir = join(f_video_path, 'i', '*.png')
            image_files_color = sorted(glob.glob(img_path_color))
            image_files_ir = sorted(glob.glob(img_path_ir))

            info[f_video] = {'image_files_color': image_files_color, 'image_files_ir': image_files_ir, 'gt': gt, 'name': f_video}

    elif 'RGB-T234' in dataset:
        base_path = join('/home/cscv/Documents/lsl/dataset/', dataset)
        json_path = join('/home/cscv/Documents/lsl/dataset/', dataset + '.json')
        jsons = json.load(open(json_path, 'r'))
        testingvideos = list(jsons.keys())
        ...
        for f_video in father_videos:
            ...
            f_video_path = join(base_path, f_video)
            # ground truth
            gt_path = join(f_video_path, 'visible.txt')
            gt = np.loadtxt(gt_path, delimiter=',')
            # get img file
            img_path_color = join(f_video_path, 'visible', '*.jpg')
            img_path_ir = join(f_video_path, 'infrared', '*.jpg')
            image_files_color = sorted(glob.glob(img_path_color))
            image_files_ir = sorted(glob.glob(img_path_ir))

            info[f_video] = {'image_files_color': image_files_color, 'image_files_ir': image_files_ir, 'gt': gt,
                             'name': f_video}
    elif 'LasHeR' in dataset:
        base_path = join('/media/cscv/d00985a0-c3e6-4ffa-9546-88c861db5ce3/02_Dataset/LasHeR/testingset')
        json_path = join('/home/cscv/Documents/lsl/dataset/', dataset + '.json')
        jsons = json.load(open(json_path, 'r'))
        testingvideos = list(jsons.keys())
        father_videos = sorted(os.listdir(base_path))
