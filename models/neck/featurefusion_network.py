...
class FeatureFusionNetwork(nn.Module):
    ...
    def forward(self, src_temp, src_search1, src_search2 , w, mask_temp=None, mask_search=None, pos_temp=None, pos_search=None):
        ...
        src_search2 = src_search2.flatten(2).permute(2, 0, 1)
        # pos_search = pos_search.flatten(2).permute(2, 0, 1)
        # mask_temp = mask_temp.flatten(1)
        # mask_search = mask_search.flatten(1)

        memory_temp, memory_search = self.encoder(src1=src_temp, src2=src_search1, src3=src_search2,
                                                  src1_key_padding_mask=mask_temp,
                                                  src2_key_padding_mask=mask_search,
                                                  pos_src1=pos_temp,
                                                  pos_src2=pos_search)
        hs = torch.add(memory_temp, memory_search)
        hs = self.norm1(hs)
        tgt = self.linear2(self.dropout(self.activation(self.linear1(hs))))
        hs = hs + self.dropout(tgt)
        hs = self.norm2(hs)
...
class Encoder(nn.Module):
    ...
    def forward(self, src1, src2, src3,
        ...
        for layer in self.layers:
            output1, output2 = layer(src1, src2, src3, src1_mask=src1_mask,
                                     src2_mask=src2_mask,
...
class FeatureFusionLayer(nn.Module):
    ...
    def forward_post(self, src1, src2, src3,
                     src1_mask: Optional[Tensor] = None,
                     src2_mask: Optional[Tensor] = None,
                     src1_key_padding_mask: Optional[Tensor] = None,
                     src2_key_padding_mask: Optional[Tensor] = None,
        ...
        src12 = self.multihead_attn1(query=self.with_pos_embed(src1, pos_src1),
                                   ...
                                   key_padding_mask=src2_key_padding_mask)[0]
        src22 = self.multihead_attn2(query=self.with_pos_embed(src1, pos_src2),
    ...
    def forward(self, src1, src2, src3,
        ...
        return self.forward_post(src1, src2, src3, src1_mask, src2_mask,
